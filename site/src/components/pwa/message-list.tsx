...
function EventCard({ event }: { event: TimelineEvent }) {
  ...
  return <article className={`pwa-message ${event.kind}`}>
    <div className="pwa-message-label">{label}{event.kind === "tool" ? <span className={`pwa-interrupted ${event.status}`}>{event.status}</span> : null}{collapsible ? <IconButton className="pwa-message-toggle" type="button" onClick={() => setExpanded((value) => !value)} aria-label={expanded ? "Collapse message" : "Expand message"} title={expanded ? "Collapse message" : "Expand message"} aria-expanded={expanded}><ChevronDown size={14} /></IconButton> : null}</div>
...
function PendingCard({ pending, onRetryUnknown, onCancelQueued }: { pending: TimelinePending; onRetryUnknown?: (clientRequestId: string) => void; onCancelQueued?: (clientRequestId: string) => void }) {
  ...
  return <article className="pwa-message user pending"><div className="pwa-message-label">{label}<span className="pwa-streaming"><span /> {pending.delivery === "unknown_delivery" ? "delivery unknown" : pending.delivery}</span>{pending.delivery === "unknown_delivery" && onRetryUnknown ? <IconButton className="pwa-pending-retry" type="button" onClick={() => onRetryUnknown(pending.clientRequestId)} aria-label="Retry delivery" title="Retry delivery"><RefreshCw size={14} /></IconButton> : null}{queued && onCancelQueued ? <IconButton className="pwa-pending-retry" type="button" onClick={() => onCancelQueued(pending.clientRequestId)} aria-label="Cancel queued message" title="Cancel queued message"><X size={14} /></IconButton> : null}</div><div className="pwa-user-blocks">{pending.text ? <p>{pending.text}</p> : null}{pending.images?.map((image, index) => <img className="pwa-message-image" key={index} src={`data:${image.mime};base64,${image.data}`} alt={`User attachment ${index + 1}`} />)}</div><time>{new Date(pending.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time></article>;
}
...
function PartialCard({ partial }: { partial: TimelinePartial }) {
  ...
  return <article className={`pwa-message ${partial.kind} partial`}><div className="pwa-message-label">{label}<span className="pwa-streaming"><span /> streaming</span></div>{text.trim() ? partial.kind === "assistant" ? <div className="pwa-stream-text">{text}</div> : <p>{text}</p> : null}</article>;
}
