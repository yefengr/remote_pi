> 历史边界：本文件是原加固实现的历史快照，不作为当前执行或项目状态真源；现行设备验收由[当前 Plan 64](../active/20260824-pwa-hardening.md)承接，且本快照不表示移动发布门槛已通过。当前状态见 [ROADMAP](../../ROADMAP.md)。

# 计划 64 — PWA 规范化与 Serwist 加固

状态：已确认

基于：`docs/plans/completed/20260822-pwa-scope.md`、`docs/plans/completed/20260822-pwa-architecture.md`

本文件记录 Remote Pi PWA 规范化与 Service Worker 加固的确认结果。它补充并在 PWA Service Worker 方案上取代 `docs/plans/completed/20260822-pwa-architecture.md` 中的“小型自定义 Service Worker”方案；不修改 Relay、`pi-extension` 或消息历史同步协议。
