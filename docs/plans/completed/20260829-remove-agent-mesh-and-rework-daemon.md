...
| 既有计划 | 本计划后的状态 |
|---|---|
| [Plan 19 — Agent Network](20260521-agent-network.md) | Agent-to-agent 网络整体取消；本地 UDS broker、agent tools 和 skill 删除。 |
| [Plan 24 — Mesh Membership](20260523-mesh-membership.md) | 跨电脑 Owner membership、mesh storage、`/mesh` API 和 topology 删除；每台电脑单独配对。 |
| [Plan 25 — PC Mesh Bootstrap](20260524-pc-mesh-bootstrap.md) | 跨电脑 Pi-to-Pi 路由、ACK、broker remote 和 bridge 删除。 |
| [Plan 34 — Mesh Reliable Delivery](20260601-mesh-reliable-delivery-passive-presence.md) | 仅属于 agent mesh 的可靠投递与 presence 语义删除。 |
| [Plan 35 — Leaderless Mesh](20260606-mesh-leaderless-redesign.md) | 已废弃，继续仅作为历史记录。 |
| [Plan 38 — Structured Mesh Identity](20260606-mesh-structured-identity.md) | `(pc, cwd, name)` agent address 和 `#N` 冲突分配删除。 |
| [Plan 41 — Room per cwd/name](20260608-room-per-cwd-name.md) | `(cwd, name)` room 身份删除；改为显式 endpoint 身份。 |
| [Plan 51 — Cross-PC Mesh Routing](20260722-cross-pc-mesh-routing-hardening.md) | 跨 PC agent routing 整体删除。 |
| [Plan 17 — Rooms](20260521-rooms.md) | 多运行入口能力保留，但 `room` 术语、cwd 派生身份和旧协议被 endpoint 模型取代。 |
| [Plan 26 — Daemon Mode](20260524-daemon-mode.md) | supervisor 架构保留；registry、启动策略、扩展加载、状态和重试模型被本文重写。 |
| [Plan 39 — Daemon Cron](20260607-daemon-cron.md) | Cron 保留，但必须消费新的 desired state/readiness；删除 `wake` 绕过语义，不能自行启动 stopped/blocked daemon。 |
