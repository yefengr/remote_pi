...
export function ComposerCommandMenuPanel({
  ...
  if (view === "models") {
    return <div className="pwa-command-menu-panel" role="group" aria-label="Change model">
      <UnstyledButton className="pwa-command-back" type="button" role="menuitem" onClick={onBack}><ArrowLeft size={16} /><span>Back</span></UnstyledButton>
      <div className="pwa-command-menu-heading">Change model</div>
      {models.length ? models.map((model) => {
        const selected = isCurrentModel(model, currentModel);
        return <UnstyledButton className="pwa-command-row pwa-command-choice" type="button" role="menuitem" key={`${model.provider}:${model.id}`} disabled={actionDisabled} onClick={() => onSetModel(model)}>
          <Cpu size={16} />
  ...
  if (view === "thinking") {
    return <div className="pwa-command-menu-panel" role="group" aria-label="Thinking level">
      <UnstyledButton className="pwa-command-back" type="button" role="menuitem" onClick={onBack}><ArrowLeft size={16} /><span>Back</span></UnstyledButton>
      <div className="pwa-command-menu-heading">Thinking level</div>
      {COMPOSER_THINKING_LEVELS.map((level) => {
        const selected = thinking === level;
        return <UnstyledButton className="pwa-command-row pwa-command-choice" type="button" role="menuitem" key={level} disabled={actionDisabled} onClick={() => onSetThinking(level)}>
          <Gauge size={16} />
  ...
  return <div className="pwa-command-menu-panel">
    <UnstyledButton className="pwa-command-row" type="button" role="menuitem" disabled={newSessionDisabled} onClick={onNewSession}>
      <FilePlus2 size={16} />
      <span className="pwa-command-copy"><code>/new</code><small>New session</small></span>
    </UnstyledButton>
    <UnstyledButton className="pwa-command-row" type="button" role="menuitem" disabled={newSessionDisabled} onClick={onCompactSession}>
      <Minimize2 size={16} />
      <span className="pwa-command-copy"><code>/compact</code><small>Compact context</small></span>
    </UnstyledButton>
    <UnstyledButton className="pwa-command-row" type="button" role="menuitem" disabled={actionDisabled} onClick={onOpenModels}>
      <Cpu size={16} />
    ...
    <UnstyledButton className="pwa-command-row" type="button" role="menuitem" disabled={actionDisabled} onClick={onOpenThinking}>
      <Gauge size={16} />
